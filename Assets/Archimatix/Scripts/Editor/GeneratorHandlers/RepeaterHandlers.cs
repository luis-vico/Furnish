...
namespace AX.GeneratorHandlers
{
	...
	public class UVProjectorHandler : GeneratorHandler3D
	{
		...
		//public override void drawBoundsHandles(Matrix4x4 consumerM, bool forceDraw=false)
		{
			...
			Vector3 posTY = new Vector3((gener.x2 + gener.x1) / 2, gener.topY, 0);
			Vector3 posBY = new Vector3((gener.x2 + gener.x1) / 2, gener.baseY, 0);
			...
			if (gener.P_TopY != null)
				Handles.DrawLine(new Vector3(gener.x1, gener.topY, 0), new Vector3(gener.x2, gener.topY, 0));
			if (gener.P_BaseY != null)
				Handles.DrawLine(new Vector3(gener.x1, gener.baseY, 0), new Vector3(gener.x2, gener.baseY, 0));
			...
			if (gener.P_TopY != null)
			{
				...
				if (EditorGUI.EndChangeCheck())
				{
					...
					gener.P_TopY.initiatePARAMETER_Ripple_setFloatValueFromGUIChange(pos.y);
			...
			if (gener.P_BaseY != null)
			{
				...
				if (EditorGUI.EndChangeCheck())
				{
					...
					gener.P_BaseY.initiatePARAMETER_Ripple_setFloatValueFromGUIChange(pos.y);
